...
export class BookingService {
  ...
  bookingLists: Booking[] = [new Booking(1, "Juan Pérez", "+34 600 123 456", "juan.perez@example.com", 4, "Prefiere mesa cerca de la ventana.", new
  ...
  deleteBooking(id: number) {
    let index = this.bookingLists.findIndex((booking) => booking.id == id);
    this.bookingLists.splice(index, 1);
  }

  getBookingById(id: number): Booking | null {
    for (let i = 0; i < this.bookingLists.length; i++) {
      if (id == this.bookingLists[i].id) {
        return this.bookingLists[i]
      }
    }
    return null;
  }
  ...
  addBooking(booking: Booking) {
    const index = this.bookingLists.findIndex(bookinglist => bookinglist.id === booking.id);


    console.log(index)
    if (index <= -1) {
      booking.id = Math.trunc(Math.random() * 1000) + 1;
      this.bookingLists.push(booking);
      console.log(booking);
    } else {
      this.bookingLists[index] = booking;
    }
  }
